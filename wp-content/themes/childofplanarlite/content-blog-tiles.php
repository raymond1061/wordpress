...
<?php if ( $count == 1 || $count == 2 ) { ?>
		...
		<figure class="tile" style="background: <?php echo esc_attr( get_theme_mod( 'planar_additional_color', '#fff' ) ); ?><?php $thumbnail = wp_get_attachment_image_src( get_post_thumbnail_id(), 'planar-quadratum' );  if( !empty($thumbnail) ) { ?> url(<?php echo $thumbnail[0]; ?>) no-repeat; background-size: cover; background-position:50%<?php } ?>;">
